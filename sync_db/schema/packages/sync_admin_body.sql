...
create or replace package body sync_admin as
    ...
    begin
        ...
        num_offline_licenses := sync_util.get_num_offline_licenses(true);

        if num_offline_licenses is not null then
            select count(*)
            into add_device.num_devices
            from device;

            if num_devices >= num_offline_licenses then
                sync_error.raise(sync_error.out_of_licenses);
            end if;
        end if;

        device_uuid := sync_util.generate_uuid;
    ...
    begin
        ...
        disk_uuid := sync_util.generate_uuid;
        shared_char := sync_util.boolean_to_char(shared);
        read_only_char := sync_util.boolean_to_char(read_only);
    ...
    begin
        ...
        repo_uuid := sync_util.generate_uuid;
    ...
    begin
        ...
        vm_uuid := sync_util.generate_uuid;
    ...
    begin
        ...
        vm_instance_uuid := sync_util.generate_uuid;
    ...
    function get_licensing return sys_refcursor is
        num_offline_licenses synchronizer.num_offline_licenses%type;
        licensing sys_refcursor;
    begin
        set transaction read only;

        sync_util.check_database_state;

        num_offline_licenses := sync_util.get_num_offline_licenses;

        open licensing for
            select case when num_offline_licenses is null then 'online'
                        else 'offline'
                        end licensing_mode,
    ...
    begin
        ...
        removed_char := sync_util.boolean_to_char(removed);
    ...
    begin
        ...
        if replace then
            delete from vm_config
            where vm_uuid = modify_vm_config.vm_uuid;
        end if;
    ...
    begin
        ...
        begin
            update vm_instance
            set vm_instance_name = modify_vm_instance_name.vm_instance_name
            where vm_instance_uuid = modify_vm_instance_name.vm_instance_uuid;
        exception
            when sync_error.cannot_update_null then
                case sync_error.get_violated_column
                    when 'vm_instance.vm_instance_name' then
                        sync_error.raise(sync_error.vm_inst_name_required);
                    else
    ...
    procedure remove_vm_instance(
        ...
        hard_removal_char varchar2(1);
    begin
        sync_util.check_database_state;

        lock_vm_instance_row(vm_instance_uuid);

        hard_removal_char := sync_util.boolean_to_char(hard_removal);

        update vm_instance
        set removed = sync_util.true_char,
            hard_removal = remove_vm_instance.hard_removal_char
