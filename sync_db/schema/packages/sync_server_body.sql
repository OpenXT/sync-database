...
create or replace package body sync_server as
    ...
    procedure get_device_state(
        ...
        online_licensing boolean;
        online_licensing_char varchar2(1);
        license_ok boolean;
    begin
        ...
        online_licensing := sync_util.get_num_offline_licenses is null;
        online_licensing_char := sync_util.boolean_to_char(online_licensing);
        ...
        open device_info for
            select release,
                   ...
                   license_hash,
                   online_licensing_char online_licensing
            from device
        ...
        if online_licensing then
            -- This check is not strictly necessary, since
            -- update_device_license performs the same check after locking the
            -- device and device_license row, but it avoids the overhead of
            -- locking the rows in the common case that the license doesn't
            -- need updating.
