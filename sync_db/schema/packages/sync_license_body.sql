...
create or replace package body sync_license as
    ...
    procedure check_online_licensing is
    begin
        if sync_util.get_num_offline_licenses(true) is not null then
            sync_error.raise(sync_error.offline_licensing);
        end if;
    end;
    ...
    procedure set_num_offline_licenses(
        num_offline_licenses in number
    ) is
    begin
        sync_util.check_database_state;
        ...
        begin
            update synchronizer
            set num_offline_licenses = set_num_offline_licenses.num_offline_licenses;
        exception
            when sync_error.check_constraint_violated then
                case sync_error.get_violated_constraint
                    when 'synchronizer_ck1' then
                        sync_error.raise(sync_error.num_licenses_invalid,
                                         num_offline_licenses);
                    else
                        raise;
                end case;

            when sync_error.precision_exceeded then
                sync_error.raise(sync_error.num_licenses_invalid,
                                 num_offline_licenses);
        end;
