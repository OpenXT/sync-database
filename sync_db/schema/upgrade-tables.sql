...
-------------------------------------------------------------------------------
-- For upgrade from schema version 3. Remove section when no longer needed.
-------------------------------------------------------------------------------
...
begin
    if not column_exists('vm_instance', 'hard_removal') then
        execute immediate
            'alter table vm_instance add
             (
                 hard_removal varchar2(1)
             )';
    end if;

    if not constraint_has_column('vm_instance_ck3', 'hard_removal') then
        execute immediate
            'alter table vm_instance
                 drop constraint vm_instance_ck3';
    end if;

    if not constraint_exists('vm_instance_ck3') then
        execute immediate
            'alter table vm_instance
                 add constraint vm_instance_ck3
                 check ((removed = ''f'' and hard_removal is null) or
                        (removed = ''t'' and hard_removal in (''t'', ''f'')))';
    end if;
end;
/
...
begin
    if not table_exists('synchronizer') then
        execute immediate
            'create table synchronizer
             (
                 num_offline_licenses number(9)
             )';
    end if;

    if not constraint_exists('synchronizer_ck1') then
        execute immediate
            'alter table synchronizer
                 add constraint synchronizer_ck1
                 check (num_offline_licenses >= 0)';
    end if;
end;
/
