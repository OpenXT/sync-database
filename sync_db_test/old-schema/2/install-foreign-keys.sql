...
alter table device_user
    add constraint device_user_fk1
    foreign key (device_uuid)
    references device (device_uuid)
    deferrable;

alter table device_user
    add constraint device_user_fk2
    foreign key (user_uuid)
    references user_ (user_uuid)
    deferrable;
...
alter table vm_instance
    ...
    foreign key (device_uuid, user_uuid)
    references device_user (device_uuid, user_uuid)
    deferrable;
