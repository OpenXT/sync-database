...
create or replace package body sync_admin as
        ...
        if device_name is not null then
            ...
            open devices for
                ...
                where user_uuid = list_devices.user_uuid and
                      device_user.removed = sync_util.false_char
                order by device_uuid;
    ...
    begin
        ...
        open devices for
            ...
            select device_uuid,
                   ...
                   user_name
            from device
                 left outer join du using (device_uuid)
                 left outer join user_ using (user_uuid)
                 left outer join report_device using (device_uuid)
        ...
        if user_name is not null then
            ...
            open users for
                ...
                where device_uuid = list_users.device_uuid and
                      user_.removed = sync_util.false_char and
    ...
    function list_users_ui(
        user_name_contains in varchar2
    ) return sys_refcursor is
        pattern varchar2(2048);
        users sys_refcursor;
    begin
        ...
        if user_name_contains is not null then
            pattern := '%' ||
                       escape_like(lower(
                            list_users_ui.user_name_contains)) || '%';
        end if;
        ...
        open users for
            select user_uuid,
                   ...
                   auth_token
            from user_
            where removed = sync_util.false_char and
                  (pattern is null or
                   lower(user_name) like pattern escape '!')
            order by user_uuid;
    ...
    function list_vm_insts_for_dev_user_ui(
        device_uuid in varchar2,
        user_uuid   in varchar2,
        config      in varchar2_list
    ) return sys_refcursor is
    ...
    begin
        ...
        check_device_user_exists(device_uuid, user_uuid);
        ...
        open vm_instances for
            with vc1 as
                 (select vm_uuid,
                         value
                  from vm_config
                  where daemon = list_vm_insts_for_dev_user_ui.daemons(1) and
                        key = list_vm_insts_for_dev_user_ui.keys(1)),
                 vc2 as
                 (select vm_uuid,
                         value
                  from vm_config
                  where daemon = list_vm_insts_for_dev_user_ui.daemons(2) and
                        key = list_vm_insts_for_dev_user_ui.keys(2))
            select vm_instance_uuid,
                   vm_instance_name,
                   vc1.value config_value_1,
                   vc2.value config_value_2
            from vm_instance
                 join device_user using (device_uuid, user_uuid)
                 left outer join vc1 using (vm_uuid)
                 left outer join vc2 using (vm_uuid)
            where device_uuid = list_vm_insts_for_dev_user_ui.device_uuid and
                  user_uuid = list_vm_insts_for_dev_user_ui.user_uuid and
                  vm_instance.removed = sync_util.false_char
            order by vm_instance_uuid;
    ...
    begin
        ...
        from vm_instance
        where user_uuid = remove_user.user_uuid and
              removed = sync_util.false_char and
              rownum <= 1;
    ...
    begin
        ...
        open device_user_errors for
            ...
            from user_
                 join device_user using (user_uuid)
            where user_.removed = sync_util.true_char and
                  device_user.removed = sync_util.false_char
            order by device_uuid,
                     user_uuid;
        ...
        open vm_instance_errors for
            ...
            where device_user.removed = sync_util.true_char and
                  vm_instance.removed = sync_util.false_char
            order by vm_instance_uuid;
